...
namespace Domain.Specs.for_Improvable.when_registering.when_applying_business_rules
{
    ...
    public class and_the_recipe_does_not_exist : given.a_business_validator_for<and_the_recipe_does_not_exist>
    {
        static RegisterImprovable register_improvable;
        static IEnumerable<ValidationResult> results;

        Establish context = () => register_improvable = constants.get_command((_)=> _.Recipe = "Some weird recipe that we've never heard of");

        Because of = () => results = business_validator.ValidateFor(register_improvable);

        It should_be_invalid = () => results.ShouldBeInvalid();
        It should_indicate_the_recipe_does_not_exist = () => results.Any(_ => _.ErrorMessage.ToLower().Contains("no recipe")).ShouldBeTrue();
    }
}