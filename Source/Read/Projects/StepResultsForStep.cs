    ...
    public class StepResultsForStep : IQueryFor<StepResult>
    {
        ...
        public IQueryable<StepResult> Query
        {
            get
            {
                ...
                {
                    var stepsFiles = Directory.GetFiles(stepsPath, "*.json").OrderBy(_ => _);
                    var stepFile = stepsFiles.Select(_ =>
                    {
                        int number = 0;
                        var numberFromFileName = Path.GetFileNameWithoutExtension(_);
                        int.TryParse(numberFromFileName, out number);
                    ...
                    }).FirstOrDefault(_ => _.Number == Number);

                    if( stepFile != null )
                    {
                        var content = File.ReadAllText(stepFile.Filename);
